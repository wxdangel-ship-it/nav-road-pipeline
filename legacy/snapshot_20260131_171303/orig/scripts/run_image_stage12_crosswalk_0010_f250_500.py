...
CFG_DEFAULT = Path("configs/image_stage12_crosswalk_0010_f250_500.yaml")
...
def main() -> int:
    ...
    run_id = now_ts()
    run_dir = Path("runs") / f"image_stage12_crosswalk_0010_250_500_{run_id}"
    ...
    for d in (tables_dir, qa_dir, stage1_dir, stage2_masks_dir, stage2_overlay_dir):
    ...
    inputs = processor(images=roi_img, text=[prompts], return_tensors="pt").to(device)
    with torch.no_grad():
    ...
    dets = []
    for bbox, score, label in zip(full_boxes, full_scores, full_labels):
    ...
    sam2_predictor.set_image(np.array(seed_img))
    ...
    for bbox in full_boxes:
        ...
        for m in masks[:max_masks]:
            ...
            if float(np.sum(mask_bin)) < min_area:
                continue
    ...
    for idx, mask in enumerate(seed_masks):
        ...
        _write_mask(out_path, mask, seed_img.size)

    _draw_overlay_stage1(seed_img, full_boxes, full_scores, seed_masks, stage1_dir / "overlay_stage1.png")
    ...
    report_lines = [
        "# Image Stage1+Stage2 Crosswalk Primitive Evidence (0010 f250-500)",
        "",
        f"- run_id: {run_id}",
        f"- drive_id: {drive_id}",
        ...
        f"- qa montage: qa/qa_montage_stage2.png",
