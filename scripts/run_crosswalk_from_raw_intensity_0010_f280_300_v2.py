...
LOG = logging.getLogger("crosswalk_from_raw_intensity_0010_f280_300_v2")
...
def _raster_extent(transform: rasterio.Affine, shape: Tuple[int, int]) -> Tuple[float, float, float, float]:
    xmin = float(transform.c)
    ymax = float(transform.f)
    xres = float(transform.a)
    yres = float(abs(transform.e))
    xmax = xmin + xres * shape[1]
    ymin = ymax - yres * shape[0]
    return xmin, xmax, ymin, ymax


def _plot_score(arr: np.ndarray, transform: rasterio.Affine, out_path: Path, title: str) -> None:
    fig, ax = plt.subplots(figsize=(6, 6))
    extent = _raster_extent(transform, arr.shape)
    ax.imshow(arr, origin="upper", cmap="gray", vmin=0.0, vmax=1.0, extent=extent)
    ax.set_title(title)
    ax.set_axis_off()
...
def _plot_mask(arr: np.ndarray, transform: rasterio.Affine, out_path: Path, title: str) -> None:
    fig, ax = plt.subplots(figsize=(6, 6))
    extent = _raster_extent(transform, arr.shape)
    ax.imshow(arr, origin="upper", cmap="gray", extent=extent)
    ax.set_title(title)
    ax.set_axis_off()
    fig.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
...
def _iter_polys(geom: Optional[object]) -> List[Polygon]:
    if geom is None:
        return []
...
def _overlay_color(
    ...
    fig, ax = plt.subplots(figsize=(6, 6))
    extent = _raster_extent(transform, score.shape)
    ax.imshow(score, origin="upper", cmap="gray", vmin=0.0, vmax=1.0, extent=extent)
    for poly in _iter_polys(truth_poly):
        x, y = poly.exterior.xy
        ax.plot(x, y, color="green", linewidth=1.2)
    if draw_stripes:
        for poly in stripe_polys:
            if poly.is_empty:
                continue
            x, y = poly.exterior.xy
            ax.plot(x, y, color="yellow", linewidth=1.0)
    if draw_candidates:
        for poly in candidate_polys:
            ...
            if not mrr.is_empty:
                mx, my = mrr.exterior.xy
                ax.plot(mx, my, color="red", linewidth=1.0, linestyle="--")
    if metrics_text:
        ax.text(
            0.02,
            0.98,
...
def _overlay_coverage(
    truth_poly: Optional[object],
    candidate_union: Optional[object],
    out_path: Path,
    title: str,
) -> None:
    fig, ax = plt.subplots(figsize=(6, 6))
    ...
    for poly in _iter_polys(candidate_union):
        x, y = poly.exterior.xy
        ax.fill(x, y, color="red", alpha=0.4)
    if truth_poly is not None and candidate_union is not None:
        inter = truth_poly.intersection(candidate_union)
        for poly in _iter_polys(inter):
            x, y = poly.exterior.xy
...
def main() -> None:
    cfg_path = Path("configs/crosswalk_from_raw_intensity_0010_f280_300_v2.yaml")
    ...
    run_id = now_ts()
    run_dir = Path("runs") / f"crosswalk_from_raw_intensity_0010_f280_300_v2_{run_id}"
    ...
    _plot_score(bev.score, bev.transform, img_dir / "score_preview.png", "score")
    _plot_mask(bev.score_mask.astype(np.uint8), bev.transform, img_dir / "score_mask_preview.png", "score mask")
    ...
    candidate_union = unary_union(candidate_polys) if candidate_polys else Polygon()
    truth_area = float(truth_geom.area)
    candidate_area = float(candidate_union.area) if not candidate_union.is_empty else 0.0
    overlap_area = float(truth_geom.intersection(candidate_union).area) if candidate_area > 0 else 0.0
    coverage_truth = overlap_area / truth_area if truth_area > 0 else 0.0
    coverage_candidate = overlap_area / candidate_area if candidate_area > 0 else 0.0
    coverage_metrics = {
        ...
        "is_multipolygon": bool(candidate_union.geom_type == "MultiPolygon"),
        "stripe_count": stripe_count,
    }
    write_json(tbl_dir / "coverage_metrics.json", coverage_metrics)

    metrics_text = (
        f"truth_area={truth_area:.1f} m2\n"
        f"cand_area={candidate_area:.1f} m2\n"
        f"overlap={overlap_area:.1f} m2\n"
        f"coverage={coverage_truth:.2f}"
    )
    _overlay_color(
        bev.score,
        bev.transform,
        ...
        candidate_polys,
        stripe_polys,
        img_dir / "candidate_overlay_color.png",
        "candidates",
    ...
    _overlay_color(
        ...
        [],
        stripe_polys,
        img_dir / "stripes_overlay_color.png",
        "stripes",
    ...
    coverage_truth = float(coverage_metrics["coverage_truth"])
    candidate_count = int(coverage_metrics["candidate_count"])
    if candidate_count == 0 or coverage_truth < 0.30:
        status = "FAIL"
    elif coverage_truth >= 0.60 and candidate_count >= 1:
        status = "PASS"
    ...
    report = [
        "# Crosswalk from raw intensity 0010 f280-300 v2",
        "",
        f"- status: {status}",
        ...
        f"- coverage_truth: {coverage_truth:.3f}",
        f"- coverage_candidate: {coverage_metrics['coverage_candidate']:.3f}",
        "",
        "## outputs",
        ...
        f"- {relpath(run_dir, img_dir / 'candidate_overlay_color.png')}",
        f"- {relpath(run_dir, img_dir / 'stripes_overlay_color.png')}",
        f"- {relpath(run_dir, img_dir / 'coverage_overlay.png')}",
        f"- {relpath(run_dir, tbl_dir / 'coverage_metrics.json')}",
    ]
    write_text(run_dir / "report.md", "\n".join(report) + "\n")
