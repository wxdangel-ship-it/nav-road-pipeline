...
if "%MONITOR_DRIVE%"=="" for /f %%i in ('.venv\Scripts\python.exe -c "import yaml;print(yaml.safe_load(open('configs/crosswalk_monitor.yaml','r',encoding='utf-8'))['drive_id'])"') do set "MONITOR_DRIVE=%%i"
if "%MONITOR_START%"=="" for /f %%i in ('.venv\Scripts\python.exe -c "import yaml;print(yaml.safe_load(open('configs/crosswalk_monitor.yaml','r',encoding='utf-8'))['frame_start'])"') do set "MONITOR_START=%%i"
if "%MONITOR_END%"=="" for /f %%i in ('.venv\Scripts\python.exe -c "import yaml;print(yaml.safe_load(open('configs/crosswalk_monitor.yaml','r',encoding='utf-8'))['frame_end'])"') do set "MONITOR_END=%%i"
...
if not "%MONITOR_START%"=="" set "EXTRA=%EXTRA% --frame-start %MONITOR_START%"
if not "%MONITOR_END%"=="" set "EXTRA=%EXTRA% --frame-end %MONITOR_END%"
